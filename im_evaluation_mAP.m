...
%     disp(['Computing the performance on (', num2str(query_num), '):       ']);
    for i = 1:query_num
        ...
        this_img_groundtruth_data = img_groundtruth_data(idx,:);
        ...
        if opts.match.qe_negative > 0
            ...
            [indexs_negative, ~] = get_nn(this_query_X_negative, img_features_data, opts.match.metric);
...
%         distances = sum(xy, 2);
    ...
    if metric == 9
        % Nearest neighbor object matching
        n = size(img_features_data,1);
        top = 10;
        this_query_X(this_query_X==0) = [];
        d = ones(1,top);
        distances = ones(1, n);
        for i = 1:n
            objects = img_features_data(i,:);
            objects(objects==0) = [];
            for j = 1:top
                dist = abs(objects(j) - this_query_X);
                d(j) = min(dist);
            end
            distances(i) = min(d);
        end
...
%    save([opts.match.rank_path, rank_file_name], 'indexs_name');
    ...
    gt_num = (size(good_set,1) + size(ok_set, 1));
    old_recall = 0.0;
    old_precision = 1.0;
    map = 0.0;
    tp = 0;    
    ...
    for i = 1:length(rank_set)
        ...
        if strcmp(datasets, 'ukbench')
            ...
            map = map +  (abs(recall - old_recall)) * ((precision + old_precision) / 2.0);
        end
        old_recall = recall;
