function img_groundtruth_data = im_evaluation_load_groundtruth(opts, img_query_name, path)
    ...
    % output:
    %   img_groundtruth_data: the ground data, n * 5 cell type
    
    img_groundtruth_data = cell(1,5);   % query image name,good image names,ok image names,junk image names,datasets name
%     disp(['load images groundtruth data from ', path, '(total: ', num2str(size(img_query_name,2)), ')      ']);
    if strcmp(opts.datasets.name, 'holidays') || strcmp(opts.datasets.name, 'holidays_upright')
