function report_eval = im_evaluation(opts, fdata)
    ...
    opts.eval.img_groundtruth_data = strcat('img_groundtruth_data_', opts.datasets.name);

    %% calculate mAP
    [mAP, pre, rec, auc] = im_evaluation_mAP(opts, fdata);
    disp([opts.features.cross_model, '_', opts.features.net, '_', opts.datasets.name, '_', ...
        opts.features.pipeline_model, '_', num2str(opts.features.dimension), ...
        ': mAP: ', num2str(mAP,'%.4f'),...
